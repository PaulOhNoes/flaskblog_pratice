...
@users.route('/register', methods=['GET', 'POST'])  # allows post and get methods
def register():
    ...
    if form.validate_on_submit():
        ...
        # alert message that returns two variables; message and category.
...
def login():
    ...
    if form.validate_on_submit():
        ...
        # check if both user/email and user password exist b4 login
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            login_user(user, remember=form.remember.data)

            # next_page saves the text string of the website
            # url when a user try to access account page w/o logging in
            next_page = request.args.get('next')

            return redirect(next_page) if next_page else redirect(url_for('main.home'))
        else:
            flash("Login Failed. Please check parameters.", 'danger')
...
def user_posts(username):
    ...
    # starting page, separate posts by 5 per page
